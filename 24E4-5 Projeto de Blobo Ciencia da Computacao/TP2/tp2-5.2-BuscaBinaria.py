...
class ArvoreBinaria:
    ...
    def parallel_search(self, target):
        search_subtree(self, self.raiz, target)

    def search_subtree(self, node, target, found):
        if node is None or found.value:
            return
        if node.value == target:
            found.value = True
            return
        search_subtree(self, node.left, target, found)
        search_subtree(self, node.right, target, found)
